<template>
  <div class="pan-and-zoom">
    <div
      ref="slot-content"
      class="slot-content"
      @mouseover="gainKeyFocus"
      @mouseout="loseKeyFocus"
      @mousedown="startOffset"
      @mouseup="endOffset"
      @mousemove="trackOffset"
      @wheel="scrollZoom"
    >
      <grid-fill
        :viewx="gridX"
...
export default {
  ...
  mounted() {
    ...
    self.resizeObserver = new ResizeObserver(() => {
      ...
      if (!self.fitted) {
        setTimeout(self.fit, 0)
      }
    })
    if (self.$el) {
      self.resizeObserver.observe(self.$el)
    }
    self.$emit('apiReady', self)
  },
  destroyed() {
  ...
  methods: {
    ...
    scrollZoom(ev) {
      ev.preventDefault()
      const { zoom } = this
      const newZoom = zoom + (ev.deltaY * -0.001)
    ...
    reset() {      
      const { bestFit, recaculateSizes } = this
      if (bestFit) {
        Object.assign(this, bestFit)
        recaculateSizes()
      }
    },
    fit() {
      ...
      const {
        ...
        trackOffsetX, trackOffsetY,
        viewSizeX, viewSizeY,
        setZoom
      } = this
      const zw = viewSizeX / zoomWidth
      const zh = viewSizeY / zoomHeight
      const requiredZoom = Math.min(zw, zh)

      if (zoomWidth === 0 || zoomHeight === 0) {
        setTimeout(this.fit, 0)
        setZoom(1.0)
      } else {
        // console.log('Fit diagram:', { zw, zh, viewSizeX, viewSizeY, zoomWidth, zoomHeight, requiredZoom })
